...
def clear_writing(writing):
    ...
    setence_words = nltk.word_tokenize(writing)
    
    return [lemmatizer.lemmatize(word.lower()) for word in setence_words]

# Função para retornar 0 ou 1 para cada palavra
def bag_of_words(writing, words):
    """
        Pega as setenças que são limpas e cria um pacote de palavras que são usadas para classes de previsão que são baseadas nos resultados que obtivemos treinando o modelo
    """
    ...
    for setence in sentence_words:
        for i, word in enumerate(words):
            if word == setence:
                # atribui 1 na lista de palavras se a palavra atual estiver na posição da frase
                bag[i] = 1
    return(np.array(bag))

def class_prediction(writing, model):
    """
        faz a previssão da lista de palavras, usamos como limite de erro 0.25 para evitamos ovverfitting
        
        Classificamos esses resultados por força da probabilidade
    """
...
def get_response(intents, intents_json):
    ...
    for idx in list_of_intents:
        if idx['tag'] == tag:
            # caso as respostas sejam um array contendo mais de uma, usamos a função de random para pegar uma resposta randomica da nossa lista
            result = random.choice(idx['responses'])
            break
    return result

clear_writing('ola mundo')
print('setup base concluído!')